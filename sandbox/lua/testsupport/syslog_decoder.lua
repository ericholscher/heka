...
local SYSLOGBASE = l.Cg(SYSLOGTIMESTAMP, "syslog_timestamp") * l.P" " * (SYSLOGFACILITY * l.P" ")^-1 * l.Cg(IPORHOST, "logsource") * l.P" " * SYSLOGPROG 
...
payload = "Oct 30 17:09:26 victors-air Google Chrome Helper[36789] <Error>: Process unable to create connection because the sandbox denied the right to lookup com.apple.coreservices.launchservicesd and so this process cannot talk to launchservicesd. : LSXPCClient.cp #426 ___ZN26LSClientToServerConnection21setupServerConnectionEiPK14__CFDictionary_block_invoke() q=com.apple.main-thread"
...
function decode(payload)
    ...
    if setContains(keyset, "syslog_timestamp") then
        ...
        t['Timestamp'] = tonumber(ndp.when(captures['syslog_timestamp'])) * 1000000
